...
const determineShippingZone = ({ origin = {}, address = {} }) => {
...
const calculateShippingFee = ({ shippingSettings = {}, address = {}, cartTotal = 0 }) => {
  const threshold = Math.max(0, Number(shippingSettings.freeShippingThreshold ?? 0));
  if (threshold > 0 && cartTotal > threshold) {
    return 0;
  }
  const zone = determineShippingZone({ origin: shippingSettings.origin || {}, address });
